...
const Customers = () => {
  ...
  const fetchCustomers = async () => {
    try {
      ...
      // Fetch orders for each customer from their user-specific orders collection
      await fetchCustomerOrders(customersData);
    } catch (error) {
  ...
  const fetchCustomerOrders = async (customersData) => {
    try {
      const phoneNumbersMap = {};
      const ordersCountMap = {};
      
      // Fetch orders for each customer from their specific user orders collection
      for (const customer of customersData) {
        try {
          // Fetch from user-specific orders collection: /users/{userId}/orders
          ...
          ordersCountMap[customer.id] = ordersSnapshot.size;
          
          // Try to extract phone number from orders
          if (ordersSnapshot.size > 0) {
            const firstOrder = ordersSnapshot.docs[0].data();
            const phoneFromOrder = firstOrder.phoneNumber || 
                                 firstOrder.customerPhone || 
                                 firstOrder.phone ||
                                 firstOrder.contactNumber;
            
            if (phoneFromOrder && !phoneNumbersMap[customer.id]) {
              phoneNumbersMap[customer.id] = phoneFromOrder;
            }
          }
        } catch (error) {
          console.error(`Error fetching orders for customer ${customer.id}:`, error);
          ordersCountMap[customer.id] = 0;
        }
      }
      
      // Also check the main orders collection as fallback
      try {
        const mainOrdersCollection = collection(db, 'orders');
        const mainOrdersSnapshot = await getDocs(mainOrdersCollection);
        
        mainOrdersSnapshot.docs.forEach(orderDoc => {
          const orderData = orderDoc.data();
          const customerId = orderData.customerId || orderData.userId;
          
          if (customerId) {
            // Update order count
            ordersCountMap[customerId] = (ordersCountMap[customerId] || 0) + 1;
            
            // Extract phone number if not already set
            if (!phoneNumbersMap[customerId]) {
              phoneNumbersMap[customerId] = orderData.phoneNumber || 
        ...
        console.error('Error fetching from main orders collection:', error);
      }
      
      // Update customers with order counts and phone numbers
      const updatedCustomers = customersData.map(customer => ({
        ...customer,
        totalOrders: ordersCountMap[customer.id] || customer.totalOrders || 0,
        phone: phoneNumbersMap[customer.id] || customer.phone || ''
      }));
