  ...
  const calculateOrderTotal = (order) => {
    ...
    if (order.products && Array.isArray(order.products) && order.products.length > 0) {
      return order.products.reduce((sum, product) => {
        const itemPrice = parseFloat(product.price || product.sellingPrice || product.mrp || product.cost || 0);
        const itemQuantity = parseInt(product.quantity || product.qty || product.count || 1);
    ...
    if (order.items && Array.isArray(order.items) && order.items.length > 0) {
      return order.items.reduce((sum, item) => {
        const itemPrice = parseFloat(item.price || item.sellingPrice || item.mrp || item.cost || 0);
        const itemQuantity = parseInt(item.quantity || item.qty || item.count || 1);
        return sum + (itemPrice * itemQuantity);
      }, 0);
    }
      ...
      <div className="hidden lg:block">
        {[...Array(5)].map((_, i) => (
          <tr key={i} className="border-t border-gray-700">
            <td className="px-6 py-4"><div className="h-4 bg-gray-600 rounded w-32"></div></td>
            <td className="px-6 py-4"><div className="h-4 bg-gray-600 rounded w-24"></div></td>
            <td className="px-6 py-4"><div className="h-4 bg-gray-600 rounded w-20"></div></td>
            <td className="px-6 py-4"><div className="h-6 bg-gray-600 rounded w-24"></div></td>
            <td className="px-6 py-4"><div className="h-6 bg-gray-600 rounded w-20"></div></td>
            <td className="px-6 py-4"><div className="h-4 bg-gray-600 rounded w-28"></div></td>
            <td className="px-6 py-4"><div className="h-8 bg-gray-600 rounded w-24"></div></td>
          </tr>
        ))}
      </div>
          ...
          <>
                ...
                <tbody className="divide-y divide-gray-700">
                  {filteredOrders.length === 0 ? (
                    ...
                    filteredOrders.map((order) => (
                      <tr 
                        key={order.id} 
                        className="hover:bg-gray-750 transition-colors duration-150 group"
                      >
            ...
            <div className="lg:hidden space-y-4 p-4">
              {filteredOrders.length === 0 ? (
                ...
                filteredOrders.map((order) => (
                  <div key={order.id} className="bg-gray-750 rounded-lg p-4 shadow-md border border-gray-700">
                    {/* Order Header */}
